...
if (!trackedMemberColumns.some((column) => column.name === 'last_active_at')) {
  db.exec('ALTER TABLE tracked_members ADD COLUMN last_active_at TEXT')
}
...
function serializeUser(userId) {
  ...
  const guilds = statements.listGuildsForUser.all(userId).map((guild) => ({
    ...
    ranks: statements.listRanksForGuild.all(guild.id).map((r) => ({ ...r, permissions: JSON.parse(r.permissions) })),
    trackedMembers: statements.listTrackedMembersForGuild.all(guild.id).map((member) => ({
      ...
      characters: statements.listCharactersForMember.all(member.id).map((c) => ({ ...c, isPrimary: Boolean(c.isPrimary) })),
    })),
    ...
    const transaction = db.transaction(() => {
      ...
      if (user) {
        ...
        if (existing && !existing.user_id) {
          statements.linkTrackedMemberToUser.run(request.user.id, existing.id)
        }
      }
      writeAuditLog({
        actorUserId: request.user.id,
...
app.post('/api/guilds/:guildId/tracked-members', requireAuth, (request, response, next) => {
  try {
    ...
    statements.createTrackedMember.run({
      ...
      guildId: guild.id,
      userId: null,
      rankId: request.body?.rankId || null,
      lastActiveAt: null,
      ...trackedMember,
    })
...
app.patch('/api/guilds/:guildId/tracked-members/:trackedMemberId', requireAuth, (request, response, next) => {
  try {
    ...
    statements.updateTrackedMember.run({
      ...
      guildId: guild.id,
      userId: existingTrackedMember.userId,
      rankId: request.body?.rankId !== undefined ? (request.body.rankId || null) : existingTrackedMember.rankId,
      lastActiveAt: existingTrackedMember.lastActiveAt,
      ...trackedMember,
    })
...
app.post('/api/guilds/:guildId/ranks', requireAuth, (request, response, next) => {
  try {
    ...
    statements.createRank.run({
      id,
      guildId: guild.id,
      name,
      weight: Number(request.body?.weight) || 0,
      permissions: JSON.stringify(request.body?.permissions || {}),
    })
    writeAuditLog({ actorUserId: request.user.id, action: 'rank.create', entityType: 'rank', entityId: id, details: { guildId: guild.id, name } })
...
app.patch('/api/guilds/:guildId/ranks/:rankId', requireAuth, (request, response, next) => {
  try {
    ...
    const name = sanitizeText(request.body?.name, 80)
    if (!name) throw createHttpError(400, 'Enter a rank name.')
    statements.updateRank.run({
      id: request.params.rankId,
      guildId: guild.id,
      name,
      weight: Number(request.body?.weight) || 0,
      permissions: JSON.stringify(request.body?.permissions || {}),
    })
    writeAuditLog({ actorUserId: request.user.id, action: 'rank.update', entityType: 'rank', entityId: request.params.rankId, details: { guildId: guild.id, name } })
...
app.delete('/api/guilds/:guildId/ranks/:rankId', requireAuth, (request, response, next) => {
  try {
    ...
    writeAuditLog({ actorUserId: request.user.id, action: 'rank.delete', entityType: 'rank', entityId: request.params.rankId, details: { guildId: guild.id } })
...
app.post('/api/guilds/:guildId/tracked-members/:trackedMemberId/characters', requireAuth, (request, response, next) => {
  try {
    const guild = ensureGuildEditor(request.user.id, request.params.guildId)
    const member = statements.findTrackedMemberForGuild.get(request.params.trackedMemberId, guild.id)
    if (!member) throw createHttpError(404, 'That tracked member could not be found. Refresh the page and try again.')
    ...
    statements.createCharacter.run({
      ...
      name: sanitizeText(request.body?.name, 80),
      class: sanitizeText(request.body?.class, 40),
      role: sanitizeText(request.body?.role, 40),
      level: Number(request.body?.level) || 50,
      isPrimary: request.body?.isPrimary ? 1 : 0,
    })
    writeAuditLog({ actorUserId: request.user.id, action: 'character.create', entityType: 'character', entityId: id, details: { guildId: guild.id, memberId: member.id, name: request.body?.name } })
...
app.delete('/api/guilds/:guildId/tracked-members/:trackedMemberId/characters/:characterId', requireAuth, (request, response, next) => {
  try {
    const guild = ensureGuildEditor(request.user.id, request.params.guildId)
    const member = statements.findTrackedMemberForGuild.get(request.params.trackedMemberId, guild.id)
    if (!member) throw createHttpError(404, 'That tracked member could not be found. Refresh the page and try again.')
    statements.deleteCharacter.run(request.params.characterId, member.id)
    writeAuditLog({ actorUserId: request.user.id, action: 'character.delete', entityType: 'character', entityId: request.params.characterId, details: { guildId: guild.id, memberId: member.id } })
